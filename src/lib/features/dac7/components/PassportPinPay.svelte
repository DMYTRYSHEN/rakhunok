...
<div class="mx-auto max-w-7xl space-y-6">
	{#if activeTab === 'passport_overview'}
		<!-- Banner -->
			...
			<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
				<div class="space-y-1">
					...
					<p class="text-xs text-stone-500">
						Безпарольна біометрична ідентифікація (FIDO2 / FaceID) та генерація PIN для кас
						самообслуговування
					</p>
				</div>
				...
				<div class="flex items-center gap-2">
					<span
						class="inline-flex items-center gap-1.5 rounded-full border border-rose-200 bg-rose-50 px-3 py-1 text-xs font-bold text-rose-800"
					>
						<ShieldCheck size={13} />
		...
		<div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
			...
			<div class="space-y-4 rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
				<div class="flex items-center justify-between">
					<div>
						<h3 class="text-sm font-bold text-stone-900">
							Динамічний PIN для кас самообслуговування
						</h3>
				...
				<!-- Large PIN display -->
				<div
					class="flex flex-col items-center justify-center space-y-3 rounded-2xl bg-stone-900 px-4 py-8 text-white"
				>
					<span class="text-xs font-semibold tracking-widest text-stone-400 uppercase">
						pinPay Код авторизації
					</span>
					<div
						class="flex items-center gap-3 font-mono text-4xl font-black tracking-widest text-emerald-400 sm:text-5xl"
					>
						{#each currentPin.split('') as digit}
							<span
								class="grid size-12 place-items-center rounded-xl border border-stone-700/50 bg-stone-800 shadow-inner sm:size-14"
							>
								{digit}
							</span>
						{/each}
					...
					<div class="flex items-center gap-2 pt-2 text-xs text-stone-400">
						<span class="size-2 animate-pulse rounded-full bg-emerald-500"></span>
						<span>Дійсний ще {secondsLeft} секунд</span>
					</div>
				</div>

				<div class="space-y-1 text-center text-xs text-stone-500 sm:text-left">
			...
			<div class="space-y-4 rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
				...
				<div class="space-y-3">
					<div
						class="flex items-center justify-between rounded-xl border border-stone-200 bg-stone-50 p-3.5 text-xs"
					>
						<div class="flex items-center gap-3">
							...
							<div>
								...
								<div class="text-[11px] text-stone-500">iPhone 15 Pro • Останній вхід: щойно</div>
							</div>
						</div>
						<span class="font-bold text-emerald-700">Активний</span>
					</div>
					...
					>
						<div class="flex items-center gap-3">
							...
							<div>
								...
								<div class="text-[11px] text-stone-500">TPM 2.0 • Прив'язано</div>
							</div>
						</div>
						<span class="font-bold text-emerald-700">Активний</span>
					</div>
				...
				<button
					...
					class="w-full cursor-pointer rounded-xl border border-stone-300 bg-white py-2.5 text-xs font-bold text-stone-800 transition hover:bg-stone-50"
				>
	...
	{:else if activeTab === 'passport_loyalty'}
		<!-- Loyalty Cards Hub -->
		<div class="space-y-4 rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
			<div class="flex items-center justify-between">
				<div>
					<h3 class="text-sm font-bold text-stone-900">
						Підключені картки лояльності рітейлу (КСО)
					</h3>
					<p class="text-xs text-stone-500">
						Автоматичне застосування знижок при скануванні pinPay
					</p>
				</div>
			...
			<div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
				{#each loyaltyCards as card}
					<div
						class="flex flex-col justify-between space-y-4 rounded-2xl border border-stone-200 bg-stone-50/50 p-4"
					>
						<div class="space-y-2">
							<div class="flex items-center justify-between">
								<span
									class="rounded-lg px-2 py-1 text-xs font-black {card.logoBg} {card.textColor}"
								>
									{card.networkName}
								</span>
								<span
						...
						<div
							class="flex items-center justify-between border-t border-stone-200/60 pt-3 text-xs"
						>
