<script lang="ts">
	...
	const phaseText = $derived(
		game.phase === 'over'
			? 'Гру завершено — три промахи.'
			: game.phase === 'paused'
				? 'Пауза. Продовжіть, коли будете готові.'
				: game.phase === 'running'
					? manual
						? 'Ваш хід. Оберіть кошик і натисніть «Крок».'
						: 'Ловіть яйця до падіння.'
					: 'Готові? Почніть, коли забажаєте.'
	);
	const basketText = $derived(directions.find((direction) => direction.lane === game.basket)!.name);
	const eggText = $derived(
		game.eggs
			.map(
				(egg) =>
	...
	onMount(() => {
		...
		try {
			high = boundedScore(localStorage.getItem(HIGH_SCORE_KEY));
		} catch {
			/* Private mode. */
		}
		ready = true;
		...
		function tick(now: number) {
			frame = 0;
			if (document.hidden || manual || game.phase !== 'running') {
				stop();
				return;
			}
...
<main class="fault" lang="uk">
	<header>
		<a class="brand" href={resolve('/')} aria-label="Рахунок — головна" data-sveltekit-reload
			><img src={logoUrl} alt="Рахунок" width="42" height="45" /></a
		><span class="code">HTTP {copy.status}</span>
	</header>
	<div class="content">
		<section class="message" aria-labelledby="fault-title">
			...
			<nav aria-label="Дії зі сторінкою">
				<a class="home" href={resolve('/')} data-sveltekit-reload
					>На головну <span aria-hidden="true">↗</span></a
				>
		...
		<section
			...
			{@attach keyboardScope}
			data-ready={ready}
			data-phase={game.phase}
			data-tick={game.tick}
			data-basket={game.basket}
		>
			<div class="game-heading">
				<h2 id="game-title">Поки ви тут — ловіть яйця.</h2>
				<span>01 / ПАУЗА</span>
			</div>
			<div class="console">
				...
				<div class="hardware-controls">
					<fieldset
						class="modes"
						disabled={!ready || game.phase === 'running' || game.phase === 'paused'}
					>
						<legend class="visually-hidden">Режим гри</legend>
						<button
							type="button"
							aria-label="A · 3 доріжки"
							aria-pressed={game.mode === 'A'}
							onclick={() => restart('A')}>A</button
						>
						<button
							type="button"
							aria-label="B · 4 доріжки"
							aria-pressed={game.mode === 'B'}
							onclick={() => restart('B')}>B</button
						>
					</fieldset>
					<button
						type="button"
						class="hardware-pause"
						aria-label={game.phase === 'running'
							? 'Пауза'
							: game.phase === 'paused'
								? 'Продовжити'
								: 'Почати гру'}
						disabled={!ready || game.phase === 'over'}
						onclick={togglePlay}><span aria-hidden="true">Ⅱ</span></button
					>
				</div>
				<div class="directions" role="group" aria-label="Положення кошика">
					{#each directions as direction (direction.lane)}
						<button
							type="button"
							data-lane={direction.lane}
							aria-label={`${direction.name} · ${direction.key}`}
							aria-pressed={game.basket === direction.lane}
							disabled={!ready || game.phase === 'over'}
							onclick={() => choose(direction.lane)}
							><span aria-hidden="true">{direction.arrow}</span><kbd>{direction.key}</kbd></button
						>
					{/each}
				</div>
			</div>
			...
			<div class="play-controls">
				...
				<button
					...
					title="Почати заново"
					disabled={!ready}
					onclick={() => restart()}><RotateCcw size={19} aria-hidden="true" /></button
				>
				<button
					type="button"
					class="icon-control"
					aria-label="Покроково"
					title="Покроково"
					aria-pressed={manual}
					disabled={!ready}
					onclick={toggleManual}><Footprints size={19} aria-hidden="true" /></button
				>
				{#if manual}<button type="button" disabled={game.phase !== 'running'} onclick={advance}
						>Крок</button
					>{/if}
				<button
					type="button"
					class="icon-control sound-control"
					aria-label="Звук"
					title={soundPending
						? 'Скасувати ввімкнення звуку'
						: sound
							? 'Вимкнути звук'
							: 'Увімкнути звук'}
					aria-pressed={sound}
					...
					disabled={!ready}
					onclick={toggleSound}
					>{#if sound}<Volume2 size={20} aria-hidden="true" />{:else}<VolumeX
							size={20}
							aria-hidden="true"
						/>{/if}</button
				>
			...
			<p class="game-status" role="status">
				{phaseText} Рахунок: {game.score}. Промахи: {game.misses} / 3.
			</p>
			<p class="sr-only" aria-live={manual ? 'polite' : 'off'}>Кошик: {basketText}. {eggText}</p>
			<p id="game-instructions" class="sr-only">
				Керуйте кошиком мишею, дотиком або клавішами Q, A, E, D у грі. Три промахи завершують гру. У
				покроковому режимі натискайте «Крок».
			</p>
		</section>
	</div>
	<footer>
		<span>МОЖНА ТРОХИ ПЕРЕПОЧИТИ</span><span>Навігація доступна незалежно від гри.</span>
	</footer>
</main>
...
<style>
	...
	button[aria-pressed='true'] {
		background: #e1e5d3;
		border-color: #56613e;
		box-shadow: inset 0 0 0 1px #56613e;
	}
	.fault :is(a, button):focus-visible {
		outline: 3px solid #52613f;
		outline-offset: 3px;
	}
	button:disabled {
		opacity: 0.5;
	...
	.console {
		...
		border-radius: 7% / 12%;
		background: linear-gradient(165deg, #f0e8d5, #dbcfb4 70%, #c8bba0);
		box-shadow:
			inset 0 2px 2px #fff9e9,
			inset 0 -5px 2px #b1a58d,
			0 10px 0 -4px #a69c87,
			0 16px 24px -10px #514a3f55;
	}
	.console::before {
		content: '';
		position: absolute;
		inset: 5px;
		border: 1px solid #fff8df66;
	...
	.console-caption {
		top: 6%;
		font-size: clamp(8px, 2.1cqi, 15px);
		letter-spacing: 0.19em;
	}
	.console-brand {
		bottom: 6%;
		font-size: clamp(10px, 3cqi, 21px);
	...
	.model {
		...
		left: 3%;
		top: 8%;
		font: bold clamp(7px, 1.7cqi, 11px) monospace;
		color: #6d6556;
	}
	.screen {
		position: relative;
		background: linear-gradient(120deg, #aeb497, #a0a78a 65%, #959e80);
		border: clamp(3px, 0.9cqi, 7px) solid #292b26;
		border-radius: 5px;
		padding: 3px;
		outline: 2px solid #30312b;
		outline-offset: 4px;
		box-shadow:
			inset 2px 3px 8px #333c2866,
			0 0 0 4px #d2c6aa,
			0 4px 8px #3d352655;
		overflow: hidden;
	}
	.screen::after {
		...
		inset: 0;
		pointer-events: none;
		background: linear-gradient(135deg, #ffffe512, transparent 45%);
	}
	.scoreboard {
		display: flex;
		justify-content: space-between;
	...
	.hardware-controls button {
		...
		background: transparent;
		box-shadow: none;
		font: bold clamp(8px, 2cqi, 13px) monospace;
	}
	.hardware-controls button::before {
		content: '';
		position: absolute;
		left: 4%;
		width: 38%;
		...
		background: #494941;
		box-shadow:
			inset 0 1px 1px #77786a,
			0 1px 2px #615943;
	}
	.hardware-controls button[aria-pressed='true']::before {
		background: #a83930;
	}
	.directions button {
		position: absolute;
		...
		min-height: 0;
		border-radius: 50%;
		border: 2px solid #97382e;
		background: radial-gradient(circle at 35% 28%, #e76451, #c34233 65%, #a43028);
		color: #7f2822;
		box-shadow:
			0 0 0 3px #b9ad9255,
			0 3px 2px #8b392d,
			inset 0 2px 2px #ff9d7666;
	}
	.directions button:hover:not(:disabled) {
		background: radial-gradient(circle at 35% 28%, #f87861, #ca4837 70%);
		color: #65231e;
	}
	.directions button[aria-pressed='true'] {
		background: radial-gradient(circle at 60% 70%, #cc4938, #a83329);
		box-shadow:
			0 0 0 3px #b9ad9255,
			inset 0 2px 4px #67241e;
	}
	.directions button:disabled {
		opacity: 0.7;
	}
	.directions [data-lane='UL'] {
		left: 1.6%;
		top: 36%;
	}
	.directions [data-lane='LL'] {
		left: 1.6%;
		top: 66%;
	}
	.directions [data-lane='UR'] {
		right: 1.6%;
		top: 36%;
	}
	.directions [data-lane='LR'] {
		right: 1.6%;
		top: 66%;
	}
	.directions button > span {
		font-size: clamp(11px, 3.2cqi, 23px);
	}
	kbd {
		position: absolute;
		bottom: -45%;
