...
import {
	...
	matchesTerminalOrder
} from '../services/terminal-authority.js';
...
function getInitialOrderFromWindow(): Order | null {
	if (typeof window !== 'undefined') {
		...
		if (
			win.__INITIAL_ORDER__ &&
			typeof win.__INITIAL_ORDER__ === 'object' &&
			win.__INITIAL_ORDER__.id
		) {
...
class CheckoutStore {
			...
			(typeof window !== 'undefined' &&
				(
					window as unknown as {
						__CHECKOUT_SCENARIOS__?: Record<string, Partial<ScenarioDefinition>>;
					}
	...
	private clearTerminalOrder(error = false): void {
		...
		this.errorMessage =
			'Перевірте з’єднання. Актуальний рахунок з’явиться після повторної перевірки.';
		this.isLoaded = true;
	}
	...
	async refreshTerminal(): Promise<void> {
		...
		try {
			...
			if (!current()) return;
			if (!matchesTerminalOrder(selection, result.order))
				throw new Error('Terminal detail mismatch');
			...
			if (!current()) return;
			if (!matchesTerminalOrder(confirmed, result.order))
	...
	async init(): Promise<void> {
		...
		fetchBanksCatalog()
			...
			.catch(() => {
				/* Keep bundled catalog on failure. */
			});
		...
		if (this.terminalMode) {
			...
			this.terminalTimer = setInterval(() => {
				if (
					...
					!matchesTerminalOrder(this.terminalSelection, this.order)
				) {
					this.terminalRead++;
					this.terminalBusy = false;
		...
		if (!id) {
			const pathMatch = window.location.pathname.match(
				/\/(?:pay|checkout|o|t|tag|pos)\/([a-zA-Z0-9-]{3,36})/i
			);
		...
		if (!initialOrder && !this.forcedScenario && this.terminal) {
			initialOrder = {
				...
				base_amount: 0,
				merchant: {
					display_name: 'POS Термінал',
					business_name: 'Очікування касира',
					tax_id: '',
					iban: ''
				}
			};
		...
		if (!initialOrder && this.forcedScenario) {
			if (
				this.forcedScenario === '1' ||
				this.forcedScenario === 'fixed' ||
				this.forcedScenario === 'order_classic'
			) {
				initialOrder = {
					id: 'demo-sc1',
					order_number: '№4092-A',
				...
				this.availableBonusPoints = 0;
				this.showComplianceCard = false;
			} else if (this.forcedScenario === 'fuel_station') {
				const quoteCreatedAt = new Date();
				const quoteExpiresAt = new Date(quoteCreatedAt.getTime() + 60_000);
				initialOrder = {
					id: 'demo-fuel-station',
					order_number: 'АЗС-7',
					title: 'Заправка на АЗС 7',
					type: 'open_amount',
					total_amount: 1190,
					base_amount: 1190,
					status: 'pending',
					scenario_config: {
						...
						flow_data: {
							...
							snapshot: {
								...
								revision: '1',
								observed_at: quoteCreatedAt.toISOString(),
								station: {
									id: 'station-7',
									label: 'АЗС 7',
									address: 'Київ, Берестейський проспект'
								},
								dispensers: [
									{
										id: 'pump-1',
				...
				this.forcedScenario === '3' ||
				this.forcedScenario === 'table_classic' ||
				this.forcedScenario === 'table'
			) {
				initialOrder = {
					id: 'demo-sc3',
				...
				initialOrder = {
					...
					np_tracking: {
						...
						steps: [
							...
							{
								title: 'Прямує до міста призначення (Львів)',
								time: '04.09 21:05',
								done: true,
								active: true
							},
							{
								title: 'Прибуде у Відділення №1 (вул. Городоцька, 112)',
								time: '05.09 до 18:00',
								done: false
							}
						]
	...
	private isPayable(): boolean {
		...
		return Boolean(
			...
			this.orderId &&
			(!this.terminalMode ||
				(this.terminalSelection && matchesTerminalOrder(this.terminalSelection, order))) &&
			!this.forcedScenario &&
			!/^(demo-|term-|profile-)/i.test(this.orderId) &&
	...
	async executePay(): Promise<void> {
		...
		const unchanged = () =>
			...
			window.location.href === location &&
			this.payTotalAmount === amount &&
			order?.total_amount === serverAmount &&
			order?.base_amount === serverBaseAmount &&
			(this.selectedBank?.code || 'UNJS') === bankCode &&
			this.isPayable();
		...
		try {
			...
			if (result.success !== true || !result.redirect_url)
				throw new Error(result.error || 'Ініціалізацію платежу відхилено');
		...
		const interval = setInterval(async () => {
			...
			try {
				const res = await fetch(
					`${apiBase}/api/v1/checkout/${encodeURIComponent(orderId)}/status`,
					{
						cache: 'no-store',
						redirect: 'error',
				...
				if (current() && res.ok) {
					...
					if (!current()) return;
					if (
						(data?.id != null && data.id !== orderId) ||
						(data?.order_id != null && data.order_id !== orderId)
					)
						return;
