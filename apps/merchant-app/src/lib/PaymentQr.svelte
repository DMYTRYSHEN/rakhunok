<script lang="ts">
	...
	$effect(() => {
		...
		error = false;
		loading = Boolean(currentValue);
		if (!currentValue) return;
		let active = true;
		void import('qrcode').then(async ({ default: QRCode }) => {
			if (!active) return;
			const rendered = document.createElement('canvas');
			await QRCode.toCanvas(rendered, currentValue, {
			...
			if (active) { error = true; loading = false; }
		});
		return () => { active = false; };
	});
</script>
...
<figure class="payment-qr">
	...
	<div class="payment-qr-frame" class:error aria-busy={loading}>
		<canvas bind:this={canvas} aria-label={label}></canvas>
		<span class="payment-qr-logo" class:hidden={loading || !value} aria-hidden="true">
	...
	{#if error}<p role="alert">Не вдалося створити QR-код. <button type="button" onclick={() => retry++}>Повторити</button></p>{/if}
</figure>
...
<style>
	...
	.payment-qr-logo.hidden { display: none; }
